...
TabId WebKitEngineService::addTab(const std::string & uri, const TabId * openerId, const std::string& title, bool desktopMode, bool incognitoMode)
{
    ...
    if (tabsCount() >= boost::any_cast<int>(config.get("TAB_LIMIT")))
        ...
        //FIXME
        //Before switching to correct webview, engine must first create this webview. Because of
        //that, windowCreated signal is delayed instead of calling it here. If the timer is too
        //short (eg. switching to a not yet created webview) browser will crash with segmentation
        //fault. Correct sollution to this problem would be if the engine would send us another
        //signal informing us that the webview was successfully created.
