...
#if PROFILE_MOBILE
    double mobile_scale = boost::any_cast<double>(tizen_browser::config::Config::getInstance().get("scale"));
    m_ITEM_H = boost::any_cast<int>(
            tizen_browser::config::Config::getInstance().get(CONFIG_KEY::URLHISTORYLIST_ITEM_HEIGHT));
    m_ITEM_H *= mobile_scale;
#else
    m_ITEM_H = boost::any_cast<int>(
            tizen_browser::config::Config::getInstance().get(CONFIG_KEY::URLHISTORYLIST_ITEM_HEIGHT));
#endif
    m_ITEMS_VISIBLE_NUMBER_MAX = boost::any_cast<int>(
            tizen_browser::config::Config::getInstance().get(CONFIG_KEY::URLHISTORYLIST_ITEMS_VISIBLE_NUMBER_MAX));
    m_historyItemsVisibleCurrent = m_ITEMS_VISIBLE_NUMBER_MAX;
}
        ...
        shared_ptr<services::HistoryItemVector> matchedEntries)
{
    if(!m_genlist) {
        m_genlist = createGenlist(m_parentLayout);
        m_adjustHeightCallback.set(m_genlist, m_ITEMS_VISIBLE_NUMBER_MAX,
                m_ITEM_H);
    }
    clear();
