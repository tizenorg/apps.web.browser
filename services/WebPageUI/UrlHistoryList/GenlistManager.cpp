...
GenlistManager::GenlistManager()
    ...
    , m_genlist(nullptr)
    , m_ITEM_H(boost::any_cast<int>(tizen_browser::config::Config::
...
#endif
    , m_ITEMS_VISIBLE_NUMBER_MAX(boost::any_cast<int>(tizen_browser::config::Config::
        ...
        shared_ptr<services::HistoryItemVector> matchedEntries)
{
    clear();
    m_genlist = createGenlist(m_parentLayout);
    m_adjustHeightCallback.set(m_genlist, m_ITEMS_VISIBLE_NUMBER_MAX, m_ITEM_H);
