...
void WebPageUI::createLayout()
{
    ...
    // bottom buttons
    m_bottomButtonBar = std::unique_ptr<ButtonBar>(new ButtonBar(m_mainLayout, "WebPageUI/LeftButtonBar.edj", "left_button_bar"));
...
//    m_bottomButtonBar->addAction(m_tab, "tab_button");
...
//    m_tab->setIcon("browser/toolbar_tab");
    ...
    m_addTab->setIcon("browser/add_tab");

    m_homePage = sharedAction(new Action("Home"));
    m_homePage->setIcon("browser/toolbar_home");

    m_bookmarks = sharedAction(new Action("Bookamrs"));
    m_bookmarks->setIcon("browser/toolbar_bookmark");

    m_tabs = sharedAction(new Action(_("IDS_BR_SK_TABS")));
    m_tabs->setIcon("browser/toolbar_tabs");

#if !PROFILE_MOBILE
    m_back->setToolTip(_("IDS_BR_SK_PREVIOUS"));
    m_forward->setToolTip(_("IDS_BR_SK_NEXT"));
    m_tabs->setToolTip("Tab Manager");
#endif
}
