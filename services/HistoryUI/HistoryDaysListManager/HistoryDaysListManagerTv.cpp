...
{
    std::shared_ptr<tizen_browser::services::HistoryItem> historyItem =
            std::make_shared<tizen_browser::services::HistoryItem>(
                    websiteHistoryItemData->websiteDomain,
                    websiteHistoryItemData->websiteTitle,
                    std::make_shared<tizen_browser::tools::BrowserImage>()
            );
    historyItemClicked(historyItem);
}
        ...
        const WebsiteVisitItemDataPtr websiteVisitItemData)
{
    std::shared_ptr<tizen_browser::services::HistoryItem> historyItem =
            std::make_shared<tizen_browser::services::HistoryItem>(
                    websiteVisitItemData->link,
                    websiteVisitItemData->title,
                    std::make_shared<tizen_browser::tools::BrowserImage>()
            );
    historyItemClicked(historyItem);
}
