...
void SqlStorage::updateTabTitle(Session& session, const std::string& tabId, const std::string& title)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    if (session.isValid()) {
        boost::format updateUrlTitleString("UPDATE %1%  SET %2% = ?  WHERE %3% = ?" );
        updateUrlTitleString % TABLE_URL % COL_URL_TITLE % COL_URL_TABID;

        storage::SQLTransactionScope scope(storage::DriverManager::getDatabase(m_dbString));
        std::shared_ptr<storage::SQLDatabase> connection = scope.database();

        try {
            storage::SQLQuery updateSessionNameQuery(connection->prepare(updateUrlTitleString.str()));
            updateSessionNameQuery.bindText(1, title);
            updateSessionNameQuery.bindText(2, tabId);
            updateSessionNameQuery.exec();
        } catch(storage::StorageException& e) {
            BROWSER_LOGD("[%s:%d] SQLException (%d): %s ", __PRETTY_FUNCTION__, __LINE__, e.getErrorCode(), e.getMessage());
        }
