...
std::shared_ptr<BrowserImage> WebView::captureSnapshot(int targetWidth, int targetHeight, Eina_Bool async)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    M_ASSERT(m_ewkView);
    ...
    if (async) {
        Eina_Bool result = ewk_view_screenshot_contents_get_async(m_ewkView, area, 1.0, evas_object_evas_get(m_ewkView), __screenshotCaptured, this);
        if (!result)
