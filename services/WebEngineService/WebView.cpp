...
Eina_Bool WebView::launch_contact(const char *uri, const char *protocol)
{
    ...
    if (it != uri_parts.end()) {
        if (!strcmp(protocol, "tel")) {
            if (app_control_add_extra_data(app_control, APP_CONTROL_DATA_PHONE, it->second.front().c_str()) < 0) {
                BROWSER_LOGE("Fail to app_control_add_extra_data");
                app_control_destroy(app_control);
                return EINA_FALSE;
            }
            ...
            if (app_control_add_extra_data(app_control, APP_CONTROL_DATA_EMAIL, it->second.front().c_str()) < 0) {
                BROWSER_LOGE("Fail to app_control_add_extra_data");
                app_control_destroy(app_control);
                return EINA_FALSE;
            }
