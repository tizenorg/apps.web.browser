        ...
        tizen_browser::config::SnapshotType snapshot_type)
{
    M_ASSERT(m_currentWebView);
    if (m_currentWebView)
        return m_currentWebView->captureSnapshot(width, height, snapshot_type != config::SnapshotType::SYNC, snapshot_type);
    else
...
void WebEngineService::_snapshotCaptured(std::shared_ptr<tizen_browser::tools::BrowserImage> image, config::SnapshotType snapshot_type)
{
