...
std::shared_ptr<tizen_browser::tools::BrowserImage> WebEngineService::getSnapshotData(int width, int height)
{
    M_ASSERT(m_currentWebView);
    if(m_currentWebView)
        return m_currentWebView->captureSnapshot(width, height, EINA_FALSE);
    else
        return std::make_shared<tizen_browser::tools::BrowserImage>();

}

std::shared_ptr<tizen_browser::tools::BrowserImage> WebEngineService::getSnapshotData(TabId id, int width, int height, Eina_Bool async){
