...
namespace {
    ...
    const std::string COL_CERT_ID = "cert_id";
...
HostCertList CertificateStorage::getHostCertList()
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    HostCertList hcList;
...
unsigned int CertificateStorage::addCertificateEntry(const std::string& pem, const std::string& host, int allow)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    boost::format addCertificateQueryString("INSERT OR REPLACE INTO %1% ( %2%, %3%, %4% ) VALUES ( ?, ?, ? );");
...
void CertificateStorage::updateCertificateEntry(const std::string& pem, const std::string& host, int allow)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    boost::format updateCertificateString("UPDATE %1%  SET %2% = ?, %3% = ? WHERE %4% = ?" );
    updateCertificateString % TABLE_CERTIFICATE % COL_PEM % COL_ALLOW % COL_HOST;
    storage::SQLTransactionScope scope(storage::DriverManager::getDatabase(DB_CERTIFICATE));
    std::shared_ptr<storage::SQLDatabase> connection = scope.database();
    try {
        storage::SQLQuery updateCertificateQuery(connection->prepare(updateCertificateString.str()));
        updateCertificateQuery.bindText(1, pem);
        updateCertificateQuery.bindInt(2, allow);
        updateCertificateQuery.bindText(3, host);
        updateCertificateQuery.exec();
    } catch (storage::StorageException &e) {
        BROWSER_LOGD("[%s:%d] SQLException (%d): %s ", __PRETTY_FUNCTION__, __LINE__, e.getErrorCode(), e.getMessage());
    }
}
