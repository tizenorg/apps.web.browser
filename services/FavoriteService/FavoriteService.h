...
class BROWSER_EXPORT FavoriteService
        : public tizen_browser::interfaces::AbstractFavoriteService
{
public:
    FavoriteService();
    virtual ~FavoriteService();
...
///    \todo Need to change getBookmarkId function for finding stored bookmark - check getBookmarkExists function
    ...
    void addFakeBookmark(int fake_id, const std::string & address, const std::string & tittle,
              const std::string & thumbnail_path, const std::string &  favicon_path, Evas * evas);
    void addHardcodedItemsForTDC();
};
