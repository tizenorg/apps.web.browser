...
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    Elm_Object_Item* BookmarkView = elm_gengrid_item_append(m_gengrid, m_folder_custom_item_class, item, _bookmarkCustomFolderClicked, item);
    elm_gengrid_item_selected_set(BookmarkView, EINA_FALSE);
    setEmptyGengrid(false);
}
...
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    Elm_Object_Item* BookmarkView = elm_gengrid_item_append(m_gengrid, m_folder_new_item_class, NULL, _bookmarkNewFolderClicked, NULL);
    elm_gengrid_item_selected_set(BookmarkView, EINA_FALSE);
    setEmptyGengrid(false);
}
...
void BookmarkManagerUI::addAllFolder(std::vector<std::shared_ptr<tizen_browser::services::BookmarkItem> > items, std::string folder_name)
{
    ...
    Elm_Object_Item* BookmarkView = elm_gengrid_item_append(m_gengrid, m_folder_all_item_class, folderData, _bookmarkAllFolderClicked, folderData);
    elm_gengrid_item_selected_set(BookmarkView, EINA_FALSE);
    setEmptyGengrid(false);
}
...
void BookmarkManagerUI::addMobileFolder(std::vector<std::shared_ptr<tizen_browser::services::BookmarkItem> > items, std::string folder_name)
{
    ...
    folderData->count = count;
    folderData->bookmarkManagerUI.reset(this);
    Elm_Object_Item* BookmarkView = elm_gengrid_item_append(m_gengrid, m_folder_mobile_item_class, folderData, _bookmarkMobileFolderClicked, folderData);
    elm_gengrid_item_selected_set(BookmarkView, EINA_FALSE);
    setEmptyGengrid(false);
}
