...
void auto_profile_delete_view::_items_delete_all(void)
{
	...
	_back_delete_view();
	m_manager->get_list_view()->refresh_view();
}
...
#if 0 && defined(DELETE_CONFIRM_POPUP_ENABLE)
void auto_profile_delete_view::__popup_delete_item_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	while (it) {
		...
		if (cb_data && elm_check_state_get(checkbox)) {
			auto_fill_form_item *item = apdv->m_manager->get_item_list()[cb_data->menu_index - del_count];
			m_manager->delete_auto_fill_form_item(item);
			del_count++;
		}
		it = elm_genlist_item_next_get(it);
	...
	apdv->m_manager->get_list_view()->refresh_view();
}
...
Evas_Object *auto_profile_delete_view::__content_get_cb(void *data, Evas_Object *obj, const char *part)
{
	...
	genlist_callback_data *callback_data = (genlist_callback_data *)data;
        ...
        if (!strcmp(part, "checkbox")) {
                ...
                elm_check_state_pointer_set(checkbox, &checked /*&(callback_data->is_checked)*/);
