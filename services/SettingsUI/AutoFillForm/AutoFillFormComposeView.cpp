...
void auto_fill_form_compose_view::show(void)
{
	...
	elm_object_item_part_content_set(m_main_layout, "title_left_btn", m_cancel_button);
	...
	elm_object_item_part_content_set(m_main_layout, "title_right_btn", m_done_button);
...
Eina_Bool auto_fill_form_compose_view::_apply_entry_data(void)
{
	...
	if (m_item_for_compose->get_item_compose_mode() == profile_edit) {
		...
		if (m_save_errorcode != profile_create_failed && m_save_errorcode != duplicate_profile)
			auto_fill_form_manager::get_auto_fill_form_manager()->add_item_to_list(m_item_for_compose);
	}
	auto_fill_form_manager::get_auto_fill_form_manager()->load_entire_item_list();
