...
Evas_Object *auto_fill_form_compose_view::__content_get_cb(void *data, Evas_Object *obj, const char *part)
{
	...
	if (!strcmp(part, "entry_swallow")) {
		...
		Evas_Object *entry = elm_entry_add(obj);
                elm_object_style_set(entry, "aff_entry");
		...
		Evas_Object *button = elm_button_add(obj);
                ...
                evas_object_smart_callback_add(button, "clicked", __editfield_clear_button_clicked_cb, cb_data->entry);
		/*if (!elm_entry_is_empty(entry)) {
			BROWSER_LOGE("entry is empty");
			elm_object_signal_emit(obj, "hide,clear,button,signal", "");
		}*/
                return button;
        }
