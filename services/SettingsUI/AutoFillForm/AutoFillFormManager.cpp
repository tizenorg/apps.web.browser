...
void auto_fill_form_manager::profiles_updated_cb(void* data)
{
	...
	formManager->refresh_list_view();
}

void auto_fill_form_manager::refresh_list_view()
{
	BROWSER_LOGD("-----------[%s : %d]---------", __PRETTY_FUNCTION__, __LINE__);
...
std::vector<auto_fill_form_item *> auto_fill_form_manager::load_entire_item_list(void)
{
	...
	BROWSER_LOGD("----------- List size : [%d] ---------",  m_auto_fill_form_item_list.size());
...
auto_profile_delete_view *auto_fill_form_manager::show_delete_view(void)
{
        BROWSER_LOGD("-----------[%s : %d]---------", __PRETTY_FUNCTION__, __LINE__);
        ...
        m_delete_view = new auto_profile_delete_view(this);
        m_delete_view->show(m_parent);
...
Eina_Bool auto_fill_form_manager::delete_delete_view(void)
{
        BROWSER_LOGD("-----------[%s : %d]---------", __PRETTY_FUNCTION__, __LINE__);
        if (m_delete_view)
                delete m_delete_view;
