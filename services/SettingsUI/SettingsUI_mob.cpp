...
void SettingsUI::_cache_button_clicked_cb(void *data, Evas_Object*, void*)
{
    ...
    if (data) {
        ...
        if (elm_radio_value_get(self->m_radio_group_enable) == 1) {
            ...
            elm_config_cache_edje_file_cache_size_set(elm_radio_value_get(self->m_radio_group_edje_file));
            elm_config_cache_edje_collection_cache_size_set(elm_radio_value_get(self->m_radio_group_edje_collection));
        }
        ...
        BROWSER_LOGD("[%s:%d] KAWA CHANGE", __PRETTY_FUNCTION__, __LINE__);
...
Evas_Object* SettingsUI::createCacheSlider(Evas_Object* settings_layout)
{
    ...
    radio = elm_radio_add(box);
    elm_object_text_set(radio, "1027");
    evas_object_show(radio);
    elm_box_pack_end(box, radio);
    elm_radio_state_value_set(radio, 1024);
    //evas_object_smart_callback_add(radio, "changed", radio_changed_cb, (void *)1);
    ...
    radio = elm_radio_add(box);
    elm_object_text_set(radio, "2048");
    evas_object_show(radio);
    elm_box_pack_end(box, radio);
    elm_radio_state_value_set(radio, 2048);
    //evas_object_smart_callback_add(radio, "changed", radio_changed_cb, (void *)1);

    elm_radio_group_add(radio, m_radio_group_image);

    radio = elm_radio_add(box);
    elm_object_text_set(radio, "1024");
    evas_object_show(radio);
    elm_box_pack_end(box, radio);
    elm_radio_state_value_set(radio, 1024);
    //evas_object_smart_callback_add(radio, "changed", radio_changed_cb, (void *)1);
    ...
    //edje file cache
    label = elm_label_add(box);
    elm_object_text_set(label, "Set edje file size");
    evas_object_show(label);
    elm_box_pack_end(box, label);
    ...
    elm_radio_group_add(radio, m_radio_group_edje_file);
    ...
    radio = elm_radio_add(box);
    elm_object_text_set(radio, "8");
    evas_object_show(radio);
    elm_box_pack_end(box, radio);
    elm_radio_state_value_set(radio, 8);
    //evas_object_smart_callback_add(radio, "changed", radio_changed_cb, (void *)2);
    ...
    //edje collection cache
    label = elm_label_add(box);
    elm_object_text_set(label, "Set edje collection size");
    evas_object_show(label);
    elm_box_pack_end(box, label);
    ...
    m_radio_group_edje_collection = radio;
    ...
    elm_radio_group_add(radio, m_radio_group_edje_collection);
    ...
    elm_radio_value_set(m_radio_group_interval, 32);
    elm_radio_value_set(m_radio_group_font, 0);
    elm_radio_value_set(m_radio_group_image, 2048);
    elm_radio_value_set(m_radio_group_edje_file, 32);
    elm_radio_value_set(m_radio_group_edje_collection, 64);
