...
#define FIND_ON_PAGE_ENTRY_STYLE "DEFAULT=' font_size=36 color=#000000 ellipsis=1'"
...
{
	BROWSER_LOGD("");
	//elm_theme_extension_add(NULL, FindOnPageUI_edj_path);
	m_edjFilePath = EDJE_DIR;
        m_edjFilePath.append("FindOnPageUI/FindOnPage.edj");

	//m_fop_layout = _create_main_layout(parent);
	//_set_count(0, 0);
...
FindOnPageUI::~FindOnPageUI(void)
{
	...
	elm_theme_extension_del(NULL, FindOnPageUI_edj_path);*/
...
/*void FindOnPageUI::set_landscape_mode(Eina_Bool landscape)
{
	if (landscape) {
		elm_object_signal_emit(m_fop_layout, "show,landscape-mode,signal", "");
	} else {
		elm_object_signal_emit(m_fop_layout, "show,portrait-mode,signal", "");
	}
}*/
...
Evas_Object* FindOnPageUI::createFindOnPageUILayout(Evas_Object *parent)
{
	...
	elm_object_part_text_set(entry, "elm.guide", (std::string("<font_size=36>") + BR_STRING_FIND_ON_PAGE + "</font>").c_str());
	...
	Evas_Object *close_click = elm_button_add(layout);
        ...
        evas_object_smart_callback_add(close_click, "clicked", close_clicked_cb, this);
...
void FindOnPageUI::close_clicked_cb(void* data, Evas_Object*, void*)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    FindOnPageUI *fop = (FindOnPageUI *)data;
    fop->closeFindOnPageUIClicked();
    //fop->clearItems();
}

/*void FindOnPageUI::clearItems()
{
    evas_object_hide(m_fop_layout);
}*/

/*void FindOnPageUI::__dim_area_clicked_cb(void *data, Evas_Object *obj, const char *emission, const char *source)
{
	BROWSER_LOGD("");
	RET_MSG_IF(!data, "fop  is NULL");
	FindOnPageUI *fop = (FindOnPageUI *)data;
	elm_object_signal_emit(fop->m_fop_layout, "hide,cross,button,signal", "");
	elm_entry_entry_set(fop->m_entry, "");
}*/

void FindOnPageUI::__clear_button_down_cb(void *data, Evas* /*evas*/, Evas_Object* /*obj*/, void *event_info)
{
	BROWSER_LOGD("");
...
void FindOnPageUI::__entry_changed_cb(void *data, Evas_Object *obj, void* /*event_info*/)
{
	...
	if (elm_text && strlen(elm_text))
		elm_object_signal_emit(fop->m_fop_layout, "show,cross,button,signal", "");
	else
		elm_object_signal_emit(fop->m_fop_layout, "hide,cross,button,signal", "");
...
void FindOnPageUI::__delete_clicked_cb(void *data, Evas_Object* /*obj*/, const char* /*emission*/, const char* /*source*/)
{
	BROWSER_LOGD("");
	Evas_Object *entry = (Evas_Object *)data;
