...
void CertificateContents::saveCertificateInfo(const char* /*pem*/, const char *host, HOST_TYPE type)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    //TODO: Save to db
...
void CertificateContents::updateCertificateInfo(const char* /*pem*/, const char *host, HOST_TYPE type)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    //TODO: Update in db
    m_host_cert_info[host] = type;
...
Evas_Object* CertificateContents::getContent(bool isWarning)
{
    ...
    if (isWarning && m_certi_policy) {
        const char *pem = ewk_certificate_policy_decision_certificate_pem_get(m_certi_policy);
        setCertificateData(pem);
        ewk_certificate_policy_decision_suspend(m_certi_policy);
    }

    elm_layout_content_set(m_mainLayout, "content_swallow", getCertificateContent());
    return m_mainLayout;
}

Evas_Object* CertificateContents::getCertificateContent()
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    std::string msg;
    ...
    if (!createCertificate(m_certificate_data.c_str())) {
        ...
        msg = m_cert_type == VALID ? BR_STRING_TRUSTED_AUTHORITY : BR_STRING_CERTI_MESSAGE;
    }
