...
std::map<std::string, CertificateContents::HOST_TYPE> CertificateContents::m_host_cert_info;
...
void CertificateContents::setCurrentTabCertData(const CertificateContents::CurrentTabCertificateData& data)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    if (m_genlist) {
...
Evas_Object* CertificateContents::createGenlist(Evas_Object* parent)
{
    ...
    Elm_Object_Item *it = elm_genlist_item_append(m_genlist, auth_item_class, this, NULL, ELM_GENLIST_ITEM_NONE, NULL, NULL);
    ...
    for (int i = 0; i < no_of_items; i++) {
        ...
        if (callback_data->type == ISSUED_BY_HEADER || callback_data->type == ISSUED_TO_HEADER ||
            ...
            item = elm_genlist_item_append(m_genlist, field_item_class, callback_data.get(), NULL, ELM_GENLIST_ITEM_NONE, NULL, NULL);
        } else
            item = elm_genlist_item_append(m_genlist, item_class, callback_data.get(), NULL, ELM_GENLIST_ITEM_NONE, NULL, NULL);
