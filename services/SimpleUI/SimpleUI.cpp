...
void SimpleUI::connectModelSignals()
{
    ...
    m_webEngine->setCertificateData.connect(boost::bind(&SimpleUI::setCertificateData, this, _1));
...
void SimpleUI::loadFinished()
{
    ...
    HOST_TYPE type = CertificateContents::isCertExistForHost(m_webEngine->getURI().c_str());
    ...
    switch(webConfirmation->getConfirmationType())
    {
        ...
        case basic_webengine::WebConfirmation::ConfirmationType::CertificateConfirmation:
        {
            if (m_webPageUI->stateEquals(WPUState::MAIN_WEB_PAGE)) {
                ...
                HOST_TYPE type = CertificateContents::isCertExistForHost(cert->getURI().c_str());
                ...
                else if(type != UNSECURE_HOST_ASK) {
                    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
...
void SimpleUI::setCertificateData(const char* certData)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    m_certificateContents->setCertificateData(certData);
    const char* uri = m_webEngine->getURI().c_str();
    if (CertificateContents::isCertExistForHost(uri) == HOST_ABSENT)
        CertificateContents::saveCertificateInfo(certData, uri, SECURE_HOST);
    else
        CertificateContents::updateCertificateInfo(certData, uri, SECURE_HOST);
}
...
void SimpleUI::certPopupButtonClicked(PopupButtons button, std::shared_ptr<PopupData> popupData)
{
    ...
    switch(button) {
        ...
        case CONTINUE:
        {
            ...
            const char* uri = m_webEngine->getURI().c_str();
            if (CertificateContents::isCertExistForHost(uri) == HOST_ABSENT)
                CertificateContents::saveCertificateInfo(certPopupData->cert->getPem().c_str(), uri, UNSECURE_HOST_ALLOWED);
            else
                CertificateContents::updateCertificateInfo(certPopupData->cert->getPem().c_str(), uri, UNSECURE_HOST_ALLOWED);
            break;
        }
        case BACK_TO_SAFETY:
