...
void SimpleUI::updateSecureIcon()
{
    ...
    if (uri.find("https://") == 0) {
        ...
        if (type == CertificateContents::SECURE_HOST)
            m_webPageUI->showSecureIcon(true, true);
        else if (type == CertificateContents::UNSECURE_HOST_ALLOWED)
            m_webPageUI->showSecureIcon(true, false);
        else
            m_webPageUI->showSecureIcon(false, false);
    }
    else {
        m_webPageUI->showSecureIcon(false, false);
    }
...
void SimpleUI::showCertificatePopup()
{
	...
	m_certificateContents->setCurrentTabCertData(*data);
        ...
        TabCertificateData noData = { CertificateContents::NONE, "", NULL };
	m_certificateContents->setCurrentTabCertData(noData);
    }
