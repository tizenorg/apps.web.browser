...
void SimpleUI::loadFinished()
{
    ...
    CertificateContents::HOST_TYPE type = isCertExistForHost(m_webEngine->getURI().c_str());
    BROWSER_LOGD("Host type = %d", type);
    if (type == CertificateContents::SECURE_HOST)
        m_certificateContents->setCertType(CertificateContents::VALID);
    else if (type == CertificateContents::UNSECURE_HOST_ALLOWED || type == CertificateContents::UNSECURE_HOST_UNKNOWN)
        m_certificateContents->setCertType(CertificateContents::INVALID);
}
    ...
    switch(webConfirmation->getConfirmationType())
    {
        ...
        case basic_webengine::WebConfirmation::ConfirmationType::CertificateConfirmation:
        {
            if (m_webPageUI->stateEquals(WPUState::MAIN_WEB_PAGE)) {
                ...
                CertificateContents::HOST_TYPE type = isCertExistForHost(cert->getURI().c_str());
                if (type == CertificateContents::UNSECURE_HOST_ALLOWED) {
                    ...
                    m_certificateContents->setCertPolicy(policy);
                    ...
                    popup->setTitle("Certificate confirmation");
                    popup->setContent(m_certificateContents->getContent(true));
                    popup->popupShown.connect(boost::bind(&SimpleUI::showPopup, this, _1));
                    popup->popupDismissed.connect(boost::bind(&SimpleUI::dismissPopup, this, _1));
                    popup->show();
#else
...
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    m_certificateContents->setCertificatePem(pem);
    const char* uri = m_webEngine->getURI().c_str();
    if (isCertExistForHost(uri) == CertificateContents::HOST_ABSENT)
...
void SimpleUI::certPopupButtonClicked(PopupButtons button, std::shared_ptr<PopupData> popupData)
{
    ...
    switch(button) {
        ...
        case CONTINUE:
        {
            ...
            const char* uri = m_webEngine->getURI().c_str();
            if (isCertExistForHost(uri) == CertificateContents::HOST_ABSENT)
                saveCertificateInfo(certPopupData->cert->getPem().c_str(), uri, CertificateContents::UNSECURE_HOST_ALLOWED);
            else
                updateCertificateInfo(certPopupData->cert->getPem().c_str(), uri, CertificateContents::UNSECURE_HOST_ALLOWED);
            break;
        }
        case BACK_TO_SAFETY:
        ...
        case VIEW_CERTIFICATE:
            m_certificateContents->showViewCertificatePopup();
            break;
        default:
            break;
    }
...
void SimpleUI::loadHostCertInfo()
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
...
CertificateContents::HOST_TYPE SimpleUI::isCertExistForHost(const char *host)
{
    /*Returns the host type if a cert. exists for the host */
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);

    std::string key(host);
    std::map<std::string, CertificateContents::HOST_TYPE>::const_iterator lookup_host = m_host_cert_info.find(key);
    if (lookup_host == m_host_cert_info.end())
        return CertificateContents::HOST_ABSENT;
    else
        return m_host_cert_info[key];
}

void SimpleUI::saveCertificateInfo(const char* /*pem*/, const char *host, CertificateContents::HOST_TYPE type)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    //TODO: Save to db
...
void SimpleUI::updateCertificateInfo(const char* /*pem*/, const char *host, CertificateContents::HOST_TYPE type)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    //TODO: Update in db
    m_host_cert_info[host] = type;
...
void SimpleUI::showMoreMenu()
{
    ...
    m_moreMenuUI->shouldShowViewCertificateButton(m_webEngine->getURI().find("https://") == 0);
...
void SimpleUI::showCertificatePopup()
{
    ...
    ContentPopup* popup = ContentPopup::createPopup(m_viewManager.getContent());
    m_certificateContents->setOwnerPopup(popup);
    m_certificateContents->setCertPolicy(NULL);
    ...
    popup->setContent(m_certificateContents->getContent(false));
    popup->popupShown.connect(boost::bind(&SimpleUI::showPopup, this, _1));
    popup->popupDismissed.connect(boost::bind(&SimpleUI::dismissPopup, this, _1));
    popup->show();
#endif
