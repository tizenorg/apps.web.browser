...
#include "Config.h"
#include "app_i18n.h"
#include "../TabService/TabService.h"
...
#include "../../core/Tools/BookmarkItem.h"
#include "Tools/EflTools.h"
#include "BrowserImage.h"
...
#include "../QuickAccess/DetailPopup.h"
#include "../WebPageUI/UrlHistoryList/UrlHistoryList.h"
...
{
    basic_webengine::webengine_service::WebEngineService::getInstance().suspend();
}
...
int SimpleUI::exec(const std::string& _url)
{
    ...
    if(!m_initialised){
        if (m_window.get()) {
            ...
            ViewManager::getInstance().pushViewToStack(&WebPageUI::getInstance());
#if PROFILE_MOBILE
            // Register H/W back key callback
            services::PlatformInputManager::getInstance().registerHWKeyCallback(ViewManager::getInstance().getContent());
            services::PlatformInputManager::getInstance().registerHWKeyCallback(MoreMenuUI::getInstance().getContent());
#endif
        }
...
void SimpleUI::titleChanged(const std::string& title, const std::string& tabId)
{
    ...
    WebPageUI::getInstance().setPageTitle(title);
...
void SimpleUI::connectUISignals()
{
    ...
    M_ASSERT(&WebPageUI::getInstance());
    WebPageUI::getInstance().getURIEntry().uriChanged.connect(boost::bind(&SimpleUI::filterURL, this, _1));
    WebPageUI::getInstance().getURIEntry().uriEntryEditingChangedByUser.connect(boost::bind(&SimpleUI::onURLEntryEditedByUser, this, _1));
    WebPageUI::getInstance().getUrlHistoryList()->openURLInNewTab.connect(boost::bind(&SimpleUI::onOpenURLInNewTab, this, _1));
    WebPageUI::getInstance().getUrlHistoryList()->uriChanged.connect(boost::bind(&SimpleUI::filterURL, this, _1));
    WebPageUI::getInstance().backPage.connect(boost::bind(&SimpleUI::switchViewToWebPage, this));
//    WebPageUI::getInstance().backPage.connect(boost::bind(&tizen_browser::basic_webengine::AbstractWebEngine<Evas_Object>::back, &basic_webengine::webengine_service::WebEngineService::getInstance()));
    WebPageUI::getInstance().reloadPage.connect(boost::bind(&SimpleUI::switchViewToWebPage, this));
    WebPageUI::getInstance().showTabUI.connect(boost::bind(&SimpleUI::showTabUI, this));
    WebPageUI::getInstance().showMoreMenu.connect(boost::bind(&SimpleUI::showMoreMenu, this));
//    WebPageUI::getInstance().forwardPage.connect(boost::bind(&tizen_browser::basic_webengine::AbstractWebEngine<Evas_Object>::forward, &basic_webengine::webengine_service::WebEngineService::getInstance()));
//    WebPageUI::getInstance().stopLoadingPage.connect(boost::bind(&tizen_browser::basic_webengine::AbstractWebEngine<Evas_Object>::stopLoading, &basic_webengine::webengine_service::WebEngineService::getInstance()));
//    WebPageUI::getInstance().reloadPage.connect(boost::bind(&tizen_browser::basic_webengine::AbstractWebEngine<Evas_Object>::reload, &basic_webengine::webengine_service::WebEngineService::getInstance()));
    WebPageUI::getInstance().showQuickAccess.connect(boost::bind(&SimpleUI::showQuickAccess, this));
    WebPageUI::getInstance().hideQuickAccess.connect(boost::bind(&QuickAccess::hideUI, &QuickAccess::getInstance()));
    WebPageUI::getInstance().bookmarkManagerClicked.connect(boost::bind(&SimpleUI::showBookmarkManagerUI, this));
#if PROFILE_MOBILE
//    WebPageUI::getInstance().setWebViewTouchEvents.connect(boost::bind(&tizen_browser::basic_webengine::AbstractWebEngine<Evas_Object>::setTouchEvents, &basic_webengine::webengine_service::WebEngineService::getInstance(), _1));
    WebPageUI::getInstance().hideMoreMenu.connect(boost::bind(&SimpleUI::closeMoreMenu, this));
    WebPageUI::getInstance().getURIEntry().mobileEntryFocused.connect(boost::bind(&WebPageUI::mobileEntryFocused, &WebPageUI::getInstance()));
    WebPageUI::getInstance().getURIEntry().mobileEntryUnfocused.connect(boost::bind(&WebPageUI::mobileEntryUnfocused, &WebPageUI::getInstance()));
    WebPageUI::getInstance().qaOrientationChanged.connect(boost::bind(&QuickAccess::orientationChanged, &QuickAccess::getInstance()));
#endif
    ...
    SettingsUI::getInstance().closeSettingsUIClicked.connect(boost::bind(&SimpleUI::closeSettingsUI, this));
    SettingsUI::getInstance().deleteSelectedDataClicked.connect(boost::bind(&SimpleUI::settingsDeleteSelectedData, this,_1));
    SettingsUI::getInstance().resetMostVisitedClicked.connect(boost::bind(&SimpleUI::settingsResetMostVisited, this));
    SettingsUI::getInstance().resetBrowserClicked.connect(boost::bind(&SimpleUI::settingsResetBrowser, this));
#if PROFILE_MOBILE
//    SettingsUI::getInstance().getWebEngineSettingsParam.connect(boost::bind(&basic_webengine::AbstractWebEngine<Evas_Object>::getSettingsParam, &basic_webengine::webengine_service::WebEngineService::getInstance(), _1));
//    SettingsUI::getInstance().setWebEngineSettingsParam.connect(boost::bind(&basic_webengine::AbstractWebEngine<Evas_Object>::setSettingsParam, &basic_webengine::webengine_service::WebEngineService::getInstance(), _1, _2));
    SettingsUI::getInstance().setWebEngineSettingsParam.connect(boost::bind(&storage::SettingsStorage::setParam, &services::StorageService::getInstance().getSettingsStorage(), _1, _2));
    SettingsUI::getInstance().isLandscape.connect(boost::bind(&SimpleUI::isLandscape, this));
#endif
    ...
    WebPageUI::getInstance().isLandscape.connect(boost::bind(&SimpleUI::isLandscape, this));
#endif
    ...
    M_ASSERT(&FindOnPageUI::getInstance());
    FindOnPageUI::getInstance().closeFindOnPageUIClicked.connect(boost::bind(&SimpleUI::closeFindOnPageUI, this));
    FindOnPageUI::getInstance().startFindingWord.connect(boost::bind(&SimpleUI::findWord, this, _1));
#endif
    ...
    M_ASSERT(&ZoomUI::getInstance());
    ZoomUI::getInstance().setZoom.connect(boost::bind(&SimpleUI::setZoomFactor, this, _1));
    ZoomUI::getInstance().getZoom.connect(boost::bind(&SimpleUI::getZoomFactor, this));
    ZoomUI::getInstance().scrollView.connect(boost::bind(&SimpleUI::scrollView, this, _1, _2));
}
...
void SimpleUI::initUIServices()
{
    ...
    M_ASSERT(&WebPageUI::getInstance());
    WebPageUI::getInstance().init(ViewManager::getInstance().getContent());

    M_ASSERT(&QuickAccess::getInstance());
    QuickAccess::getInstance().init(WebPageUI::getInstance().getContent());
    ...
    M_ASSERT(&MoreMenuUI::getInstance());
#if PROFILE_MOBILE
    MoreMenuUI::getInstance().init(WebPageUI::getInstance().getContent());
#else
    MoreMenuUI::getInstance().init(ViewManager::getInstance().getContent());
#endif

    M_ASSERT(&SettingsUI::getInstance());
    SettingsUI::getInstance().init(ViewManager::getInstance().getContent());
    ...
    M_ASSERT(&FindOnPageUI::getInstance());
    FindOnPageUI::getInstance().init(WebPageUI::getInstance().getContent());
#endif
    ...
    M_ASSERT(&ZoomUI::getInstance());
    ZoomUI::getInstance().init(ViewManager::getInstance().getContent());
}
...
void SimpleUI::initModelServices()
{
    ...
    M_ASSERT(WebPageUI::getInstance().getContent());
    basic_webengine::webengine_service::WebEngineService::getInstance().init(WebPageUI::getInstance().getContent());
    ...
    M_ASSERT(&services::PlatformInputManager::getInstance());
    services::PlatformInputManager::getInstance().init(m_window.get());
}
...
void SimpleUI::connectModelSignals()
{
    ...
    basic_webengine::webengine_service::WebEngineService::getInstance().uriChanged.connect(boost::bind(&URIEntry::changeUri, &WebPageUI::getInstance().getURIEntry(), _1));
    basic_webengine::webengine_service::WebEngineService::getInstance().webViewClicked.connect(boost::bind(&URIEntry::clearFocus, &WebPageUI::getInstance().getURIEntry()));
    basic_webengine::webengine_service::WebEngineService::getInstance().backwardEnableChanged.connect(boost::bind(&WebPageUI::setBackButtonEnabled, &WebPageUI::getInstance(), _1));
    basic_webengine::webengine_service::WebEngineService::getInstance().forwardEnableChanged.connect(boost::bind(&WebPageUI::setForwardButtonEnabled, &WebPageUI::getInstance(), _1));
    ...
    services::PlatformInputManager::getInstance().returnPressed.connect(boost::bind(&elm_exit));
    services::PlatformInputManager::getInstance().backPressed.connect(boost::bind(&SimpleUI::onBackPressed, this));
    services::PlatformInputManager::getInstance().escapePressed.connect(boost::bind(&SimpleUI::onEscapePressed, this));
    services::PlatformInputManager::getInstance().mouseClicked.connect(
            boost::bind(&SimpleUI::onMouseClick, this, _1, _2));
    services::PlatformInputManager::getInstance().redPressed.connect(boost::bind(&SimpleUI::onRedKeyPressed, this));
    services::PlatformInputManager::getInstance().yellowPressed.connect(boost::bind(&SimpleUI::onYellowKeyPressed, this));
...
//    services::StorageService::getInstance().getSettingsStorage().setWebEngineSettingsParam.connect(boost::bind(&basic_webengine::AbstractWebEngine<Evas_Object>::setSettingsParam, &basic_webengine::webengine_service::WebEngineService::getInstance(), _1, _2));
    services::PlatformInputManager::getInstance().menuButtonPressed.connect(boost::bind(&SimpleUI::onMenuButtonPressed, this));
...
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    services::PlatformInputManager::getInstance().registerHWKeyCallback(basic_webengine::webengine_service::WebEngineService::getInstance().getLayout());
}

void SimpleUI::unregisterHWKeyCallback()
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    services::PlatformInputManager::getInstance().unregisterHWKeyCallback(basic_webengine::webengine_service::WebEngineService::getInstance().getLayout());
}
#endif
...
void SimpleUI::switchViewToWebPage()
{
    ...
    if(basic_webengine::webengine_service::WebEngineService::getInstance().isSuspended())
        basic_webengine::webengine_service::WebEngineService::getInstance().resume();
    WebPageUI::getInstance().switchViewToWebPage(basic_webengine::webengine_service::WebEngineService::getInstance().getLayout(), basic_webengine::webengine_service::WebEngineService::getInstance().getURI(), basic_webengine::webengine_service::WebEngineService::getInstance().getTitle());
    WebPageUI::getInstance().toIncognito(basic_webengine::webengine_service::WebEngineService::getInstance().isPrivateMode(basic_webengine::webengine_service::WebEngineService::getInstance().currentTabId()));
}
...
void SimpleUI::switchViewToQuickAccess()
{
    ...
    WebPageUI::getInstance().switchViewToQuickAccess(QuickAccess::getInstance().getContent());
    basic_webengine::webengine_service::WebEngineService::getInstance().disconnectCurrentWebViewSignals();
    ViewManager::getInstance().popStackTo(&WebPageUI::getInstance());
}

void SimpleUI::switchViewToIncognitoPage()
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    WebPageUI::getInstance().switchViewToIncognitoPage();
    ViewManager::getInstance().popStackTo(&WebPageUI::getInstance());
}
        ...
        bool incognitoMode)
{
    ...
    switchToTab(tab);
    WebPageUI::getInstance().toIncognito(incognitoMode);
...
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    if(WebPageUI::getInstance().stateEquals(WPUState::QUICK_ACCESS))
        return false;
...
void SimpleUI::onBookmarkClicked(std::shared_ptr<tizen_browser::services::BookmarkItem> bookmarkItem)
{
    ...
    if(tabsCount() == 0 || !basic_webengine::webengine_service::WebEngineService::getInstance().isPrivateMode(basic_webengine::webengine_service::WebEngineService::getInstance().currentTabId())){
        ...
        WebPageUI::getInstance().switchViewToWebPage(basic_webengine::webengine_service::WebEngineService::getInstance().getLayout(), bookmarkAddress, bookmarkTitle);
        basic_webengine::webengine_service::WebEngineService::getInstance().setURI(bookmarkAddress);
        WebPageUI::getInstance().setPageTitle(bookmarkTitle);
        WebPageUI::getInstance().getURIEntry().clearFocus();
...
void SimpleUI::onReturnPressed(MenuButton *m)
{
    ...
    services::PlatformInputManager::getInstance().returnPressed.disconnect_all_slots();
    services::PlatformInputManager::getInstance().returnPressed.connect(boost::bind(&elm_exit));
    m->hidePopup();
}
...
void SimpleUI::onBackPressed()
{
    BROWSER_LOGD("[%s]", __func__);
#if !PROFILE_MOBILE
    if (ZoomUI::getInstance().isVisible()) {
        ZoomUI::getInstance().escapeZoom();
    } else
#endif
    if (m_wvIMEStatus) {    // if IME opened
...
#endif
    } else if ((ViewManager::getInstance().topOfStack() == &TabUI::getInstance()) && TabUI::getInstance().isEditMode()) {
        ...
        switchViewToQuickAccess();
    } else if ((ViewManager::getInstance().topOfStack() == &WebPageUI::getInstance())) {
        if (WebPageUI::getInstance().stateEquals(WPUState::QUICK_ACCESS)) {
...
#if PROFILE_MOBILE
    } else if ((ViewManager::getInstance().topOfStack() == &SettingsUI::getInstance()) && SettingsUI::getInstance().isSubpage()) {
        SettingsUI::getInstance().onBackKey();
#endif
    } else {
        ViewManager::getInstance().popTheStack();
    }
...
void SimpleUI::__after_rotation(void *data, Elm_Transit */*transit*/)
{
    ...
    MoreMenuUI::getInstance().resetContent();
    BookmarkFlowUI::getInstance().resetContent();
    SettingsUI::getInstance().orientationChanged();
    BookmarkManagerUI::getInstance().orientationChanged();
    WebPageUI::getInstance().orientationChanged();
    TabUI::getInstance().orientationChanged();
...
{
    WebPageUI::getInstance().setReloadButtonEnabled(enable);
}

void SimpleUI::stopEnable(bool enable)
{
    WebPageUI::getInstance().setStopButtonEnabled(enable);
}
...
void SimpleUI::loadStarted()
{
    ...
    WebPageUI::getInstance().loadStarted();
}
...
void SimpleUI::progressChanged(double progress, basic_webengine::TabId id, bool first)
{
    ...
    WebPageUI::getInstance().progressChanged(progress);
}

void SimpleUI::loadFinished()
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    WebPageUI::getInstance().loadFinished();
}
    ...
    if (!basic_webengine::webengine_service::WebEngineService::getInstance().isPrivateMode(basic_webengine::webengine_service::WebEngineService::getInstance().currentTabId()))
        services::HistoryService::getInstance().addHistoryItem(basic_webengine::webengine_service::WebEngineService::getInstance().getURI(),
                basic_webengine::webengine_service::WebEngineService::getInstance().getURI(),
                                         std::make_shared<tizen_browser::tools::BrowserImage>(),
                                         std::make_shared<tizen_browser::tools::BrowserImage>());
    WebPageUI::getInstance().loadStopped();
}

void SimpleUI::loadError()
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    WebPageUI::getInstance().switchViewToErrorPage();
}
...
void SimpleUI::filterURL(const std::string& url)
{
    ...
    if(/*url.empty() ||*/ url == HomePageURL){
        WebPageUI::getInstance().getURIEntry().changeUri("");
    } else if (!url.empty()){
    ...
    //no filtering
        ...
        if (basic_webengine::webengine_service::WebEngineService::getInstance().isPrivateMode(basic_webengine::webengine_service::WebEngineService::getInstance().currentTabId()) ||
                WebPageUI::getInstance().stateEquals(WPUState::MAIN_ERROR_PAGE))
            switchViewToWebPage();
    }
    WebPageUI::getInstance().getURIEntry().clearFocus();
}
...
void SimpleUI::onURLEntryEditedByUser(const std::shared_ptr<std::string> editedUrlPtr)
{
    ...
    int historyItemsVisibleMax =
            WebPageUI::getInstance().getUrlHistoryList()->getItemsNumberMax();
    int minKeywordLength =
            WebPageUI::getInstance().getUrlHistoryList()->getKeywordLengthMin();
    ...
    WebPageUI::getInstance().getUrlHistoryList()->onURLEntryEditedByUser(editedUrl, result);
}

void SimpleUI::onMouseClick(int x, int y)
{
    WebPageUI::getInstance().getUrlHistoryList()->onMouseClick(x, y);
}

void SimpleUI::onRedKeyPressed()
{
    WebPageUI::getInstance().onRedKeyPressed();
}

void SimpleUI::onYellowKeyPressed()
{
    WebPageUI::getInstance().onYellowKeyPressed();
}
...
void SimpleUI::showZoomUI()
{
    ...
    if(! WebPageUI::getInstance().stateEquals(WPUState::QUICK_ACCESS)) {
        ViewManager::getInstance().popStackTo(&WebPageUI::getInstance());
        WebPageUI::getInstance().showTabUI.connect(boost::bind(&SimpleUI::closeZoomUI, this));
        WebPageUI::getInstance().showMoreMenu.connect(boost::bind(&SimpleUI::closeZoomUI, this));
        ZoomUI::getInstance().show(m_window.get());
    }
}
...
void SimpleUI::closeFindOnPageUI()
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    M_ASSERT(&FindOnPageUI::getInstance());
    FindOnPageUI::getInstance().hideUI();
}
#endif
...
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    ViewManager::getInstance().pushViewToStack(&TabUI::getInstance());
...
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    if (ViewManager::getInstance().topOfStack() == &TabUI::getInstance())
        ViewManager::getInstance().popTheStack();
}
...
void SimpleUI::tabClicked(const tizen_browser::basic_webengine::TabId& tabId)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    ViewManager::getInstance().popStackTo(&WebPageUI::getInstance());
    WebPageUI::getInstance().toIncognito(basic_webengine::webengine_service::WebEngineService::getInstance().isPrivateMode(tabId));
    switchToTab(tabId);
}
...
void SimpleUI::handleConfirmationRequest(basic_webengine::WebConfirmationPtr webConfirmation)
{
    ...
    switch(webConfirmation->getConfirmationType())
    {
        ...
        {
        if (WebPageUI::getInstance().stateEquals(WPUState::MAIN_WEB_PAGE))
        {
        ...
        Evas_Object *popup_content = elm_layout_add(WebPageUI::getInstance().getContent());
...
void SimpleUI::showMoreMenu()
{
    ...
    if (evas_object_visible_get(MoreMenuUI::getInstance().getContent()))
        ...
        MoreMenuUI::getInstance().blockThumbnails(WebPageUI::getInstance().stateEquals(WPUState::QUICK_ACCESS));
        basic_webengine::webengine_service::WebEngineService::getInstance().moreKeyPressed();
        MoreMenuUI::getInstance().showUI();
    }
#else
    bool desktopMode = WebPageUI::getInstance().stateEquals(WPUState::QUICK_ACCESS) ? QuickAccess::getInstance().isDesktopMode() : basic_webengine::webengine_service::WebEngineService::getInstance().isDesktopMode();
...
void SimpleUI::tabCreated()
{
    ...
    WebPageUI::getInstance().setTabsNumber(tabs);
}
...
void SimpleUI::updateView() {
    ...
    if (tabs == 0) {
        switchViewToQuickAccess();
    } else if (!WebPageUI::getInstance().stateEquals(WPUState::QUICK_ACCESS)) {
        switchViewToWebPage();
    }
    WebPageUI::getInstance().setTabsNumber(tabs);
}
