...
void SimpleUI::connectUISignals()
{
...
#endif
    m_tabUI->newIncognitoTabClicked.connect(boost::bind(&SimpleUI::openNewTab, this, "", "", boost::none, desktop_ua, true, basic_webengine::Origin::UNKNOWN));
    ...
    m_historyUI->signalHistoryItemClicked.connect(boost::bind(&SimpleUI::onOpenURL, this, _1, _2, desktop_ua, basic_webengine::Origin::UNKNOWN));
...
void SimpleUI::openNewTab(const std::string &uri, const std::string& title,
        ...
        bool incognitoMode, basic_webengine::Origin origin)
{
...
{
    basic_webengine::Origin origin = basic_webengine::Origin::UNKNOWN;
    if (m_webPageUI->stateEquals(WPUState::QUICK_ACCESS))
        origin = basic_webengine::Origin::QUICKACCESS;
...
void SimpleUI::onOpenURL(const std::string& url, const std::string& title, bool desktopMode, const basic_webengine::Origin& origin)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    m_viewManager.popStackTo(m_webPageUI.get());
...
void SimpleUI::loadFinished()
{
    ...
    if (!m_webEngine->isPrivateMode(m_webEngine->currentTabId())) {
        m_tabService->updateTabItem(m_webEngine->currentTabId(), m_webEngine->getURI(), m_webEngine->getTitle(), m_webEngine->getOrigin());
