...
void SimpleUI::connectModelSignals()
{
    ...
    m_webEngine->setCertificatePem.connect(boost::bind(&SimpleUI::setCertificatePem, this, _1, _2));
#if PROFILE_MOBILE
...
void SimpleUI::updateSecureIcon()
{
    ...
    if (uri.find("https://") == 0) {
        show = true;
        CertificateContents::HOST_TYPE type = CertificateContents::isCertExistForHost(tools::extractDomain(uri));
...
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    TabCertificateData data = { CertificateContents::NONE, "", NULL };
    std::string pem = m_storageService->getCertificateStorage().getPemForURI(tools::extractDomain(m_webEngine->getURI()));
    if (!pem.empty())
        data = { CertificateContents::INVALID, pem, NULL };
    showCertificatePopup(data);
}

void SimpleUI::showCertificatePopup(TabCertificateData& data)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
...
void SimpleUI::setCertificatePem(std::string uri, std::string pem)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
...
void SimpleUI::certPopupButtonClicked(PopupButtons button, std::shared_ptr<PopupData> popupData)
{
    ...
    switch(button) {
        ...
        {
            TabCertificateData data = { CertificateContents::INVALID, certPopupData->cert->getPem(), NULL };
            showCertificatePopup(data);
            break;
        }
        default:
