...
int SimpleUI::getBookmarkFolderCount(int folder_id)
{
    return m_favoriteService->getBookmarkFolderCount(folder_id);
}
...
#if PROFILE_MOBILE
    m_bookmarkManagerUI->addFolders(getBookmarks(tizen_browser::services::ROOT_FOLDER_ID));
...
void SimpleUI::onBookmarkAllFolderClicked()
{
    ...
    m_bookmarkManagerUI->addDetails(getBookmarks(tizen_browser::services::ALL_BOOKMARKS_ID),getBookmarkFolderName(tizen_browser::services::ALL_BOOKMARKS_ID),getBookmarkFolderCount(tizen_browser::services::ALL_BOOKMARKS_ID));
...
void SimpleUI::onBookmarkMobileClicked()
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    m_bookmarkManagerUI->getDetailsContent();
    m_bookmarkManagerUI->addDetails(getBookmarks(tizen_browser::services::ROOT_FOLDER_ID),getBookmarkFolderName(tizen_browser::services::ROOT_FOLDER_ID),getBookmarkFolderCount(tizen_browser::services::ROOT_FOLDER_ID));
    m_bookmarkManagerUI->showDetailsUI();
}

void SimpleUI::onBookmarkCustomFolderClicked(std::shared_ptr<tizen_browser::services::BookmarkItem> bookmarkItem)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    int folderId = bookmarkItem->getId();
    m_bookmarkManagerUI->getDetailsContent();
    m_bookmarkManagerUI->addDetails(getBookmarks(folderId),getBookmarkFolderName(folderId),getBookmarkFolderCount(folderId));
    m_bookmarkManagerUI->showDetailsUI();
}
#endif
