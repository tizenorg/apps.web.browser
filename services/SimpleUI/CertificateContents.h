...
public:
    ...
    void setCertPolicy(Ewk_Certificate_Policy_Decision* policy) { m_certi_policy = policy; }
    void setCertificateData(const char *cert_data) { m_certificate_data.assign(cert_data); }
    void setCertType(CERT_TYPE type) { m_cert_type = type; }
    Eina_Bool isValidCertificate() const { return m_cert_type == VALID; }

    static HOST_TYPE isCertExistForHost(const char *host);
    static void saveCertificateInfo(const char *pem, const char *host, HOST_TYPE type);
    static void deleteAllSavedCertificates(void);
    static void updateCertificateInfo(const char *pem, const char *host, HOST_TYPE type);

private:

    Eina_Bool createCertificate(const char *cert_data);
    Evas_Object* getCertificateContent();
