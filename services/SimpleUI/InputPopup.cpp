...
void InputPopup::createLayout()
{
    ...
    for(auto it = m_bad_words.begin(); it != m_bad_words.end(); ++it) {
        if(m_input.compare(*it) == 0) {
...
void InputPopup::_entry_changed(void* data, Evas_Object *, void *)
{
    ...
    if (data != nullptr) {
        ...
        bool dissable = false;
        for(auto it = inputPopup->m_bad_words.begin(); it != inputPopup->m_bad_words.end(); ++it) {
            if(text.compare(*it) == 0) {
                dissable = true;
                break;
            }
        }
