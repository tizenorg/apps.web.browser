...
MobilePopup::MobilePopup(Evas_Object* parent)
    : m_parent(parent)
    , m_layout(nullptr)
    ...
    , m_content(nullptr)
{
    ...
    m_edjFilePath.append("SimpleUI/MobilePopup.edj");
...
void MobilePopup::dismiss()
{
    popupDismissed(this);
}

void MobilePopup::onBackPressed()
{
    dismiss();
}

void MobilePopup::_response_cb(void* data,
                                  Evas_Object* obj,
                                  void* /*event_info*/)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    auto self = static_cast<MobilePopup*>(data);
...
void MobilePopup::setTitle(const std::string& title)
{
    this->m_title = title;
    elm_object_translatable_part_text_set(m_layout, "title_text", m_title.c_str());
}

void MobilePopup::setContent(Evas_Object* content)
{
    m_content = content;   
    elm_object_content_set(m_scroller, content);
}

void MobilePopup::addButton(const PopupButtons& button, bool dismissOnClick)
{
    ...
    elm_object_part_text_set(btn, "elm.text", buttonsTranslations[button].c_str());
...
void MobilePopup::_layout_resize_cb(void* data, Evas* /*e*/, Evas_Object* /*obj*/, void* /*event_info*/)
{
    BROWSER_LOGD("[%s:%d] ", __PRETTY_FUNCTION__, __LINE__);
    ...
    if (y < MAX_HEIGHT)
        ...
        evas_object_size_hint_max_set(self->m_scroller, w, MAX_HEIGHT);
}
