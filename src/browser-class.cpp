...
void Browser_Class::ewk_view_deinit(Evas_Object *ewk_view)
{
	...
	evas_object_smart_callback_del(ewk_view, "filesystem,permission,request",
					Browser_View::__ewk_view_filesystem_permission_request_cb);
...
//					Browser_View::__request_certificate_confirm_cb);
	...
	evas_object_smart_callback_del(ewk_view, "usermedia,permission,request",
					Browser_View::__request_usermedia_cb);
...
void Browser_Class::ewk_view_init(Evas_Object *ewk_view)
{
	...
	evas_object_smart_callback_add(ewk_view, "filesystem,permission,request",
					Browser_View::__ewk_view_filesystem_permission_request_cb, m_browser_view);
...
//					Browser_View::__request_certificate_confirm_cb, m_browser_view);
	...
	evas_object_smart_callback_add(ewk_view, "usermedia,permission,request",
					Browser_View::__request_usermedia_cb, m_browser_view);
...
void Browser_Class::destroy_active_view(void)
{
	BROWSER_LOGD("[%s]", __func__);

	if (m_browser_view->m_option_header_find_word_layout)
		m_browser_view->__destroy_findonpage_mode(m_browser_view);
#if defined(FEATURE_MOST_VISITED_SITES)
	if (m_browser_view->m_most_visited_sites)
		m_browser_view->__destroy_mostvisit_editmode();
#endif
}
